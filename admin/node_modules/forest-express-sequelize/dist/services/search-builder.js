...
function SearchBuilder(model, opts, params, fieldNamesRequested) {
  ...
  function selectSearchFields() {
    ...
    const searchAssociationNames = _.map(searchAssociationFields, function (association) {
    ...
    const fieldsSimpleNotFound = _.xor(searchFields, _.map(fields, function (field) {
      return field.field;
    }));

    const fieldsAssociationNotFound = _.xor(_.map(searchAssociationFields, function (association) {
      return association.split('.')[0];
    }), _.keys(associations));
  ...
  this.perform = function (associationName) {
    ...
    if (parseInt(params.searchExtended, 10)) {
      _.each(associations, function (association) {
        if (!fieldNamesRequested || fieldNamesRequested.includes(association.as) || fieldNamesRequested.find(function (fieldName) {
          return fieldName.startsWith(`${association.as}.`);
        })) {
            ...
            _.each(fieldsAssociation, function (field) {
              ...
              if (field.field === schemaAssociation.idField) {
                if (field.type === 'Number') {
                  ...
                  condition = getStringExtendedCondition(modelAssociation.rawAttributes[field.field], params.search, column);
