...
function ApimapFieldBuilder(model, column, options) {
  ...
  function isRequired() {
    ...
    return column._autoGenerated !== true && (column.allowNull === false || Boolean(column.primaryKey));
  }

  function getValidations(automaticValue) {
    const validations = []; // NOTICE: Do not inspect validation for autogenerated fields, it would
  ...
  this.perform = function () {
    ...
    const canHaveDynamicDefaultValue = ['Date', 'Dateonly'].indexOf(schema.type) !== -1 || column.type instanceof DataTypes.UUID;
    const isDefaultValueFunction = typeof column.defaultValue === 'function' || canHaveDynamicDefaultValue && typeof column.defaultValue === 'object';
